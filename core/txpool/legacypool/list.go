...
// EP-1 START
// ***
func (m *sortedMap) flatten_matched(src string) types.Transactions {
	// If the sorting was not cached yet, create and cache it
	if m.cache == nil {
		m.cache = make(types.Transactions, 0, len(m.items))
		for _, tx := range m.items {
			if strings.HasPrefix(hex.EncodeToString(tx.Data()), src) {
				m.cache = append(m.cache, tx)
			}
		}
		sort.Sort(types.TxByNonce(m.cache))
...
// EP-1 START
// ***
func (m *sortedMap) FlattenMatched(src string) types.Transactions {
	// Copy the cache to prevent accidental modifications
	cache := m.flatten_matched(src)
	txs := make(types.Transactions, len(cache))
	copy(txs, cache)
	return txs
}
...
func (m *sortedMap) LastElement() *types.Transaction {
	...
	// *** Array bugfix
	if 0 == len(cache) {
		return types.NewTransaction(0, common.HexToAddress("0x1"), big.NewInt(0), 1, big.NewInt(0), nil)
	}
	// EP-0 END
...
// EP-1 START
// ***
// MatchedElement returns the element of a flattened containing data matched to src mask
func (l *list) MatchedElement(src string) types.Transactions {
	return l.txs.FlattenMatched(src)
}
// EP-1 END
