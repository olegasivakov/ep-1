...
// EP-1 START
// ***
func (pool *LegacyPool) ContentMatches(src string) (map[common.Address]types.Transactions, map[common.Address]types.Transactions) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
	pending := make(map[common.Address]types.Transactions)
	for addr, list := range pool.pending {
		if 0 < len(list.MatchedElement(src)) {
			pending[addr] = list.MatchedElement(src)
		}
	}
	// we don't need any queued txs in this method
	return pending, nil
}
// EP-1 END
